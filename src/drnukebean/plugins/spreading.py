...
def spreading(entries, options_map, config_str):
    ...
    if not isinstance(config_obj, dict):
        errors.append(
            ConfigError(
                ...
                "Invalid configuration for spreading plugin; skipping.",
                None,
            )
        )
    ...
    if "liability_acc_base" not in config_obj:
        errors.append(
            ConfigError(
                data.new_metadata("<spreading>", 0),
                "spreading plugin: 'liability_acc_base' is missing in the paramters; skipping",
                None,
            )
        )
        return entries, errors
...
def spread(entry, config_obj):
    ...
    open_directive = data.Open(
        entry.meta, datetime.date(1970, 1, 1), claim_account, [income_posting.units.currency], None
    )
    ...
    # list of dates
    dates = pd.date_range(
        entry.meta["p_spreading_start"], periods=n_divides, freq=entry.meta["p_spreading_frequency"]
    )
