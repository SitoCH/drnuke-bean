    ...
    if not isinstance(config_obj, dict):
        errors.append(
            ConfigError(
                ...
                "Invalid configuration for budgeting plugin; skipping.",
                None,
            )
        )
...
def budget(entry, config_obj):
    ...
    # list of dates
    dates = pd.date_range(start=start, periods=repeats, freq=freq)
    ...
    dropkeys = ["p_budgeting_start", "p_budgeting_frequency", "p_budgeting_times"]
